...
async function main() {
  ...
  if (seed.length !== flatSanity.length) flag(`διαφορά πλήθους: seed=${seed.length} sanity=${flatSanity.length}`)
  ...
  for (const si of flatSanity) {
    ...
    if (!seedIt) { flag(`Sanity item χωρίς αντιστοιχία στο seed: "${si.nameEl}"`); continue }
    ...
    if (combined !== norm(seedIt.price)) flag(`τιμή διαφέρει "${si.nameEl}": sanity="${combined}" seed="${seedIt.price}"`)
  ...
  console.log('\n── 4. Καρτέλες ──')
  const menus = await query<any[]>(`*[_type=="menu"]|order(order asc){labelEl,"k":key.current,order}`)
