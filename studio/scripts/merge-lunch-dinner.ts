...
const CAT_ORDER = ['cat.appetizers.0', 'cat.appetizers.1', 'cat.mains.0', PASTA_CAT, 'cat.desserts.0']
...
async function run() {
  ...
  const veganItems: string[] = await client.fetch(`*[_type=="menuItem" && references("${VEGAN_CAT}")]._id`)
  ...
  for (const id of CAT_ORDER) {
    tx = tx.patch(id, {
      set: { menu: { _type: 'reference', _ref: 'menu.lunch-dinner' }, orderRank: catRank.toString() },
    })
  ...
  console.log(`✓ Lunch / Dinner: ${CAT_ORDER.length} κατηγορίες, ${veganItems.length} vegan→ζυμαρικά, διαγράφηκαν ${OLD_MENUS.length} tabs + 1 κατηγορία`)
}
