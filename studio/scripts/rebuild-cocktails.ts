...
const CATS: Cat[] = [
  { key:'signature', title:'Signature', items:[
    X('Strawberry Sour',13,'Bond gin, strawberry, balsamic fig, vegan foam, Vermouth di Torino'),
    ...
    X('Mastic Passion',14,'Bond gin, green tea, passion, caramelized milk, foam mastic and ginger'),
    X('Maniana Colada',13,'Pampero blend, banana, caramel, coconut, lime'),
  ]},
  { key:'no-alcohol', title:'No Alcohol', items:[
    X('0% Garibaldi',10,'0% bitter, orange oleo, fake foam'),
    X('0% Paloma',10,'0% Gin, lime, agave, grapefruit soda'),
  ]},
  { key:'spritz', title:'Spritz', items:[
    X('Summer Skin',10,'Mastic tears, Aperol, peach, Fever Tree Pink Grapefruit Soda, Gancia Prosecco'),
  ...
  { key:'gin-tonic', title:'Gin & Tonic', items:[
  ...
  { key:'classic', title:'Classic', items:[
...
async function run(){
  ...
  for (const c of CATS){
    ...
    tx = tx.createOrReplace({ _id:catId, _type:'category', titleEl:c.title, titleEn:c.title, menu:{_type:'reference',_ref:MENU}, orderRank:cRank.toString(), hidden:false })
    c.items.forEach((it,i)=>{
      iRank = iRank.genNext()
      tx = tx.createOrReplace({ _id:`item.cocktails.${c.key}.i${i}`, _type:'menuItem', nameEl:it.name, nameEn:it.name,
        descEl:it.desc, descEn:it.desc, category:{_type:'reference',_ref:catId}, orderRank:iRank.toString(),
        available:true, hidden:false, prices:[{_type:'priceVariant',_key:'p0',amount:it.price}] })
    })
  }
  await tx.commit({ visibility:'async' })
  const n = CATS.reduce((a,c)=>a+c.items.length,0)
  console.log(`✓ Cocktails: ${CATS.length} κατηγορίες, ${n} προϊόντα (σβήστηκαν ${oldCats.length}/${oldItems.length})`)
}
