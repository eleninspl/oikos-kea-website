...
function extractExtra(desc: string | undefined, kw: RegExp) {
  ...
  const m = desc.match(
    new RegExp(`[—\\-]?\\s*${kw.source}\\s*:?\\s*(.+?)\\s*\\+\\s*([\\d.,]+)\\s*€`, 'i'),
  );
...
async function run() {
  const cats: any[] = await client.fetch(`*[_type=="category"] | order(orderRank asc){_id, items}`);
  ...
  console.log(
    `✓ Δημιουργήθηκαν ${created} menuItem documents από ${cats.length} κατηγορίες (items[] καθαρίστηκε)`,
  );
}
