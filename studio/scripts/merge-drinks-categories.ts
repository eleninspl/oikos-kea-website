...
async function run(){
  const items: any[] = await client.fetch(
    `*[_type=="menuItem" && references($ids)]{_id,"cat":category._ref,orderRank}`, { ids: ORDER }
  )
  const idx = (c:string)=>ORDER.indexOf(c)
  items.sort((a,b)=> idx(a.cat)-idx(b.cat) || String(a.orderRank).localeCompare(String(b.orderRank)))
  ...
  for (const it of items){
    r = r.genNext()
    tx = tx.patch(it._id, { set: { category:{_type:'reference',_ref:TARGET}, orderRank: r.toString() } })
  }
  tx = tx.patch(TARGET, { set:{ titleEl:'Ποτά', titleEn:'Drinks' } })
           .delete('cat.spirits.0').delete('cat.spirits.1').delete('cat.spirits.2')
