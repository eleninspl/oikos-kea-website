...
async function main() {
  ...
  for (const t of TABS) {
    ...
    console.log(`  ↓ ${t.el}: ${cats.length} κατηγορίες, ${cats.reduce((a, c) => a + c.items.length, 0)} προϊόντα`)
  }
  ...
  for (const t of tabs) {
    ...
    t.cats.forEach((c, ci) => {
      const items = c.items.map((it, ii) => {
        const glassAp = (it.additional_prices || []).find((a) => /ποτήρι/i.test(stripHtml(a?.description)))
        return {
          _type: 'menuItem',
          _key: `m-${t.key}-${ci}-${ii}`,
          nameEl: it.name,
          ...
          ...(it.description ? { descEl: it.description, descEn: tcache.get(it.description) || it.description } : {}),
          hidden: false,
        }
      })
      docs.push({
