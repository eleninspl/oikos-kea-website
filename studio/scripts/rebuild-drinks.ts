...
const CATS: Cat[] = [
  { key:'soft', el:'Αναψυκτικά', en:'Soft Drinks', items:[
    I('Coca Cola','Coca Cola',3.8), I('Coca Cola Zero','Coca Cola Zero',3.8),
    I('Sprite','Sprite',3.5), I('Σόδα','Soda',3.5), I('Τόνικ','Tonic',3.5),
    I('Πορτοκαλάδα Fanta Μπλέ','Orangeade Fanta Blue',3.5), I('Πορτοκαλάδα Fanta','Orangeade Fanta',3.5),
    ...
    I('Νερό Αύρα 0,5l','Avra Water 0,5l',0.5), I('Νερό Αύρα 1l','Avra Water 1l',1),
    I('Ξινόνερο 0,25l','Carbonated Water 0,25l',3), I('Ξινόνερο 1l','Carbonated Water 1l',4),
  ]},
  { key:'drinks', el:'Ποτά', en:'Drinks', items:[
    I('Ποτό Απλό','Simple Drink',8), I('Ποτό Σπέσιαλ','Special Drink',10), I('Ποτό Premium','Premium Drink',15),
  ]},
  { key:'traditional', el:'Παραδοσιακά Ποτά', en:'Traditional Drinks', items:[
    I('Ούζο Καζανιστό 200ml','Ouzo Kazanisto 200ml',12), I('Τσίπουρο Ηδωνικό 200ml','Tsipouro Idoniko 200ml',12),
  ]},
  { key:'beers', el:'Μπύρες', en:'Beers', items:[
    I('Stella Artois Draft 400ml','Stella Artois Draft 400ml',5), I('Νύμφη 330ml','Nymfi 330ml',5.5),
    I('Noam 340ml','Noam 340ml',7), I('Corona','Corona',6), I('Stella 0% alc.','Stella 0% alc.',5),
  ]},
]
...
async function run(){
  ...
  for (const c of CATS){
    ...
    c.items.forEach((it,i)=>{
      iRank = iRank.genNext()
      tx = tx.createOrReplace({ _id:`item.drinks.${c.key}.i${i}`, _type:'menuItem', nameEl:it.el, nameEn:it.en,
  ...
  console.log(`✓ Ποτά: ${CATS.length} υποκατηγορίες, ${n} προϊόντα (σβήστηκαν ${oldCats.length}/${oldItems.length})`)
}
