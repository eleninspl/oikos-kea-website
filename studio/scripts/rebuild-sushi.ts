...
const M = (el: string, en: string, price: number, dEl?: string, dEn?: string): It => ({
  el,
  en,
  price,
  dEl,
...
const CATS: Cat[] = [
  ...
  },
  {
    ...
    items: [
      M(
        ...
        'Wakame',
        9,
        'Κόκκινο τσίλι, dressing σουσαμιού, ponzu oil, φύτρες',
        'Red chili, sesame dressing, ponzu oil, cress',
      ),
    ],
  },
  {
  ...
  },
  {
    ...
    el: 'Sashimi (3 τεμ.)',
    en: 'Sashimi (3pcs)',
    items: [M('Σολομός', 'Sake', 7), M('Τόνος', 'Maguro', 8), M('Λαβράκι', 'Suzuki', 6)],
  },
  {
    key: 'nigiri',
    el: 'Nigiri (2 τεμ.)',
    ...
    items: [
      ...
      M(
        ...
        'Suzuki',
        8,
        'Μαγιονέζα yuzu kosho, πέρλες yuzu',
        'Yuzu kosho mayo, yuzu pearls',
      ),
    ],
  },
];
...
async function run() {
  ...
  console.log(
    `✓ Sushi: ${CATS.length} κατηγορίες, ${n} προϊόντα (σβήστηκαν ${oldCats.length}/${oldItems.length})`,
  );
}
run().catch((e) => {
  console.error(e);
