...
check(
  'tab keys order',
  assembled.map((t) => t.key).join(',') === 'allday,cocktails,sushi,cuisine,wines',
...
for (const tab of menuTabs) {
  ...
  check(
    `${tab.key}: ${expected} κατηγορίες`,
    byKey[tab.key].sections.length === expected,
...
check(
  'καμία κατηγορία κρασιού στο cuisine',
...
check(
  'Salmon priceAlt=Nigiri €8',
  (sake as any)?.priceAlt === 'Nigiri €8',
  (sake as any)?.priceAlt,
);
...
check(
  'Milkshake sectionPrice=€6,50',
  (milk as any)?.sectionPrice === '€6,50',
  (milk as any)?.sectionPrice,
);

console.log('\n── Item totals (χωρίς πλέον διπλομέτρηση κρασιών) ──');
const renderCount = assembled
  .flatMap((t) => t.sections)
  .reduce(
    (n, s) =>
      n + (s.items?.length ?? 0) + (s.subsections ?? []).reduce((m, x) => m + x.items.length, 0),
    0,
  );
check(
  'rendered item count = 130 (κάθε item μία φορά)',
  renderCount === 130,
  `render=${renderCount}`,
);
