...
const client = createClient({
  projectId: 's7x6np2r', dataset: 'production', apiVersion: '2022-09-09',
  token: process.env.SANITY_WRITE_TOKEN, useCdn: false,
})

type I = { el: string; en: string; price: number; dEl: string; dEn: string; extra?: { el: string; en: string; s: number } }
type C = { key: string; el: string; en: string; items: I[] }

const CATS: C[] = [
  { key: 'eggs', el: 'Αυγά', en: 'Eggs', items: [
  ...
  { key: 'healthy', el: 'Υγιεινό Πρωινό', en: 'Healthy Breakfast', items: [
    ...
    { el: 'Φαλάφελ', en: 'Falafel', price: 11.5,
  ...
  { key: 'snacks', el: 'Σνακς', en: 'Snacks', items: [
  ...
  { key: 'sweets', el: 'Γλυκές Αλχημείες', en: 'Sweet Alchemies', items: [
    ...
    { el: 'Nutella Pancake', en: 'Nutella Pancake', price: 8.5,
  ...
  { key: 'pizza-burgers', el: 'Πίτσα & Μπέργκερ', en: 'Pizza & Burgers', items: [
    ...
    { el: 'Picante', en: 'Picante', price: 13.5,
...
async function run() {
  ...
  const oldCats: string[] = await client.fetch(`*[_type=="category" && menu._ref=="menu.brunch"]._id`)
  const oldItems: string[] = await client.fetch(`*[_type=="menuItem" && references(*[_type=="category" && menu._ref=="menu.brunch"]._id)]._id`)

  const maxCatRank: string = await client.fetch(`*[_type=="category"]|order(orderRank desc)[0].orderRank`)
  const maxItemRank: string = await client.fetch(`*[_type=="menuItem"]|order(orderRank desc)[0].orderRank`)
  ...
  // Note στο tab
  tx = tx.patch('menu.brunch', { set: { noteEl: 'Σερβίρεται 08.30 – 15.00', noteEn: 'Served from 08.30 to 15.00' } })
  ...
  for (const c of CATS) {
    ...
    tx = tx.createOrReplace({
      _id: catId, _type: 'category', titleEl: c.el, titleEn: c.en,
      menu: { _type: 'reference', _ref: 'menu.brunch' }, orderRank: cRank.toString(), hidden: false,
    })
    c.items.forEach((it, idx) => {
      iRank = iRank.genNext()
      const doc: any = {
        _id: `item.brunch.${c.key}.i${idx}`, _type: 'menuItem',
        nameEl: it.el, nameEn: it.en, descEl: it.dEl, descEn: it.dEn,
        category: { _type: 'reference', _ref: catId }, orderRank: iRank.toString(),
        available: true, hidden: false,
        prices: [{ _type: 'priceVariant', _key: 'p0', amount: it.price }],
      }
      if (it.extra) doc.extras = [{ _type: 'extra', _key: 'x0', labelEl: it.extra.el, labelEn: it.extra.en, surcharge: it.extra.s }]
  ...
  console.log(`✓ Brunch ξαναχτίστηκε: ${CATS.length} κατηγορίες, ${n} προϊόντα, note προστέθηκε (σβήστηκαν ${oldCats.length} cats / ${oldItems.length} items)`)
}
